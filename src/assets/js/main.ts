...
function GlobalState(): GlobalStateInterface {
    return {
        ...
        addCartItem(product: any, quantity: any, properties: any) {
            window.Qumra.cart.addCartItem(product, quantity, properties).then((res: any) => (
                this.cardItems = res,
        ...
        updatePrductCount(_id: string, quantity: string | number) {
            window.Qumra.cart.updateQuantity(_id, quantity).then((res: any) => (
                ...
                this.cardItems.totalProductsAfterDiscount = res.totalProductsAfterDiscount
            ))
        },
        removeProduct(_id: string) {
            window.Qumra.cart.clearCartItem(_id).then((res: any) => (
                this.itemsCount = this.cardItems.items.length - 1,
                this.cardItems.items = this.cardItems.items.filter((item: any) => item._id !== _id),
                this.cardItems.totalPrice = res.totalPrice,
                this.cardItems.productDiscount = res.productDiscount,
                this.cardItems.totalProducts = res.totalProducts,
                this.cardItems.totalProductsAfterDiscount = res.totalProductsAfterDiscount
